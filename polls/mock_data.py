poll_mock_list = [
    {
		'poll_name': 'Test 4',
		'poll_type': False,
		'isPrivate': False,
		'image': [],
        'poll_data': '',
		'password': '',
		'finish_date': '2025-07-31T21:24:47.000Z',
		'bets': [
			{
				'bet_title': 'Bet 1',
				'bet_description': '',
				'bet_data': {
					'answer_options': ['opt 1', 'opt 3', 'opt 4', 'opt 5'],
					'finish_date': '2025-08-28T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "opt 1"
                            ]
                        },
                        "2": {
                            "answer": [
                                "opt 2"
                            ]
                        },
                        "4": {
                            "answer": [
                                "opt 3"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Bet 2',
				'bet_description': '',
				'bet_data': {
					'answer_options': ['opt 6', 'opt 7', 'opt 8'],
					'finish_date': '2025-07-05T21:25:36.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "opt 6"
                            ]
                        },
                        "2": {
                            "answer": [
                                "opt 7"
                            ]
                        },
                        "4": {
                            "answer": [
                                "opt 6"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			}
		]
	},
    {
		'poll_name': 'New Baby Yoda',
		'poll_type': False,
		'isPrivate': True,
		'image': [],
        'poll_data': '',
		'password': '12345',
		'finish_date': '2025-09-25T21:24:47.000Z',
		'bets': [
			{
				'bet_title': 'What will be the name of the new Baby Yoda?',
				'bet_description': '',
				'bet_data': {
					'answer_options': ['John', 'Paul', 'Ringo', 'George'],
					'finish_date': '2025-09-24T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "John"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Paul"
                            ]
                        },
                        "3": {
                            "answer": [
                                "John"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Ringo"
                            ]
                        },
                        "5": {
                            "answer": [
                                "George"
                            ]
                        },
                        "6": {
                            "answer": [
                                "Paul"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': 'John',
				'correct_answer_confirmed': True,
				'bet_hash': ''
			}
		]
	},
    {
		'poll_name': 'How will The Last of Us Part 3 end?',
		'poll_type': False,
		'isPrivate': False,
		'image': [],
        'poll_data': '',
		'password': '',
		'finish_date': '2025-09-20T21:24:47.000Z',
		'bets': [
			{
				'bet_title': 'Will Ellie be able to kill Abby?',
				'bet_description': '',
				'bet_data': {
					'answer_options': ['Yes', 'No'],
					'finish_date': '2025-09-19T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Yes"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Yes"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Yes"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Yes"
                            ]
                        },
                        "5": {
                            "answer": [
                                "No"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': True,
				'bet_hash': ''
			},
			{
				'bet_title': 'The cure will be found?',
				'bet_description': '',
				'bet_data': {
					'answer_options': ['Yes', 'No'],
					'finish_date': '2025-09-19T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Yes"
                            ]
                        },
                        "2": {
                            "answer": [
                                "No"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Yes"
                            ]
                        },
                        "4": {
                            "answer": [
                                "No"
                            ]
                        },
                        "5": {
                            "answer": [
                                "No"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': True,
				'bet_hash': ''
			},
			{
				'bet_title': 'Will Ellie die?',
				'bet_description': '',
				'bet_data': {
					'answer_options': ['Yes', 'No'],
					'finish_date': '2025-09-19T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Yes"
                            ]
                        },
                        "3": {
                            "answer": [
                                "No"
                            ]
                        },
                        "4": {
                            "answer": [
                                "No"
                            ]
                        },
                        "5": {
                            "answer": [
                                "No"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': True,
				'bet_hash': ''
			},
			{
				'bet_title': 'Will Jesse die?',
				'bet_description': '',
				'bet_data': {
					'answer_options': ['Yes', 'No'],
					'finish_date': '2025-09-19T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "No"
                            ]
                        },
                        "2": {
                            "answer": [
                                "No"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Yes"
                            ]
                        },
                        "5": {
                            "answer": [
                                "No"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': True,
				'bet_hash': ''
			}
		]
	},
    {
		'poll_name': 'Formula 1 2025',
		'poll_type': False,
		'isPrivate': False,
		'image': [],
		'password': '',
		'finish_date': '2025-12-07T21:24:47.000Z',
		'bets': [
			{
				'bet_title': 'Austrian Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-06-28T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'British Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-07-05T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Lewis Hamilton"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Lewis Hamilton"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "4": {
                            "answer": [
                                "George Russell"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Lewis Hamilton"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Belgian Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-07-26T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Lewis Hamilton"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Lando Norris"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Hungarian Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-08-02T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Lewis Hamilton"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Lando Norris"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Dutch Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-08-30T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Charles Leclerc"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Lando Norris"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Italian Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-09-06T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Charles Leclerc"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Charles Leclerc"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Lando Norris"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Azerbaijan Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-09-20T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Singapore Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-10-04T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'United States Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-10-18T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "George Russell"
                            ]
                        },
                        "2": {
                            "answer": [
                                "George Russell"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Mexico City Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-10-25T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "2": {
                            "answer": [
                                "George Russell"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'São Paulo Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-11-08T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Las Vegas Grand Prix[',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-11-21T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Lewis Hamilton"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Lando Norris"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Lando Norris"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Qatar Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-11-29T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "3": {
                            "answer": [
                                "George Russell"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Charles Leclerc"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
			{
				'bet_title': 'Abu Dhabi Grand Prix',
				'bet_description': '',
				'bet_data': {
					'answer_options': [
                        'Max Verstappen',
                        'Yuki Tsunoda',
                        'Charles Leclerc',
                        'Lewis Hamilton',
                        'Kimi Antonelli',
                        'George Russell',
                        'Lando Norris',
                        'Oscar Piastri',
                        'Lance Stroll',
                        'Fernando Alonso',
                        'Pierre Gasly',
                        'Franco Colapinto',
                        'Alex Albon',
                        'Carlos Sainz',
                        'Isack Hadjar',
                        'Liam Lawson',
                        'Oliver Bearman',
                        'Esteban Ocon',
                        'Nico Hülkenberg',
                        'Gabriel Bortoleto'
                    ],
					'finish_date': '2025-12-06T21:25:08.000Z',
					'users_answers': {
                        "1": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "2": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "3": {
                            "answer": [
                                "Oscar Piastri"
                            ]
                        },
                        "4": {
                            "answer": [
                                "Max Verstappen"
                            ]
                        },
                        "5": {
                            "answer": [
                                "Kimi Antonelli"
                            ]
                        }
                    }
				},
				'image': '',
				'bet_type': 'radio',
				'correct_answer': '',
				'correct_answer_confirmed': False,
				'bet_hash': ''
			},
		],
        'poll_data': {
            "ranking" : {
                "current": {
                    "date": "2023-10-01",
                    "users": {
                        1: {
                            "user_id": 1,
                            "user_hash": 1,
                            "user_name": "Name 1",
                            "user_image": "",
                            "points": 6,
                            "position": 2,
                            "total_bets": 50,
                            "total_wins": 30,
                            "total_losses": 10,
                        },
                        2: {
                            "user_id": 2,
                            "user_hash": 2,
                            "user_name": "Name 2",
                            "user_image": "",
                            "points": 9,
                            "position": 1,
                            "total_bets": 50,
                            "total_wins": 20,
                            "total_losses": 20,
                        },
                        3: {
                            "user_id": 3,
                            "user_hash": 3,
                            "user_name": "Name 3",
                            "user_image": "",
                            "points": 3,
                            "position": 4,
                            "total_bets": 50,
                            "total_wins": 10,
                            "total_losses": 30,
                        },
                        4: {
                            "user_id": 4,
                            "user_hash": 4,
                            "user_name": "Name 4",
                            "user_image": "",
                            "points": 0,
                            "position": 4,
                            "total_bets": 50,
                            "total_wins": 10,
                            "total_losses": 30,
                        },
                        5: {
                            "user_id": 5,
                            "user_hash": 5,
                            "user_name": "Name 5",
                            "user_image": "",
                            "points": 6,
                            "position": 2,
                            "total_bets": 50,
                            "total_wins": 10,
                            "total_losses": 30,
                        },
                    }
                },
                "history": [
                    {
                        "date": "2023-10-01",
                        "users": {
                            1: {
                                "user_id": 1,
                                "user_hash": 1,
                                "user_name": "Name 1",
                                "user_image": "",
                                "points": 6,
                                "position": 2,
                                "total_bets": 50,
                                "total_wins": 30,
                                "total_losses": 10,
                            },
                            2: {
                                "user_id": 2,
                                "user_hash": 2,
                                "user_name": "Name 2",
                                "user_image": "",
                                "points": 9,
                                "position": 1,
                                "total_bets": 50,
                                "total_wins": 20,
                                "total_losses": 20,
                            },
                            3: {
                                "user_id": 3,
                                "user_hash": 3,
                                "user_name": "Name 3",
                                "user_image": "",
                                "points": 3,
                                "position": 4,
                                "total_bets": 50,
                                "total_wins": 10,
                                "total_losses": 30,
                            },
                            4: {
                                "user_id": 4,
                                "user_hash": 4,
                                "user_name": "Name 4",
                                "user_image": "",
                                "points": 0,
                                "position": 4,
                                "total_bets": 50,
                                "total_wins": 10,
                                "total_losses": 30,
                            },
                            5: {
                                "user_id": 5,
                                "user_hash": 5,
                                "user_name": "Name 5",
                                "user_image": "",
                                "points": 6,
                                "position": 2,
                                "total_bets": 50,
                                "total_wins": 10,
                                "total_losses": 30,
                            },
                        }
                    },
                    {
                        "date": "2023-09-01",
                        "users": {
                            1: {
                                "user_id": 1,
                                "user_hash": 1,
                                "user_name": "Name 1",
                                "user_image": "",
                                "points": 3,
                                "position": 2,
                                "total_bets": 50,
                                "total_wins": 30,
                                "total_losses": 10,
                            },
                            2: {
                                "user_id": 2,
                                "user_hash": 2,
                                "user_name": "Name 2",
                                "user_image": "",
                                "points": 6,
                                "position": 1,
                                "total_bets": 50,
                                "total_wins": 20,
                                "total_losses": 20,
                            },
                            3: {
                                "user_id": 3,
                                "user_hash": 3,
                                "user_name": "Name 3",
                                "user_image": "",
                                "points": 0,
                                "position": 3,
                                "total_bets": 50,
                                "total_wins": 10,
                                "total_losses": 30,
                            },
                            4: {
                                "user_id": 4,
                                "user_hash": 4,
                                "user_name": "Name 4",
                                "user_image": "",
                                "points": 0,
                                "position": 3,
                                "total_bets": 50,
                                "total_wins": 10,
                                "total_losses": 30,
                            },
                            5: {
                                "user_id": 5,
                                "user_hash": 5,
                                "user_name": "Name 5",
                                "user_image": "",
                                "points": 3,
                                "position": 2,
                                "total_bets": 50,
                                "total_wins": 10,
                                "total_losses": 30,
                            },
                        }
                    },
                    {
                        "date": "2023-08-01",
                        "users": {
                            1: {
                                "user_id": 1,
                                "user_hash": 1,
                                "user_name": "Name 1",
                                "user_image": "",
                                "points": 3,
                                "position": 1,
                                "total_bets": 50,
                                "total_wins": 30,
                                "total_losses": 10,
                            },
                            2: {
                                "user_id": 2,
                                "user_hash": 2,
                                "user_name": "Name 2",
                                "user_image": "",
                                "points": 3,
                                "position": 1,
                                "total_bets": 50,
                                "total_wins": 20,
                                "total_losses": 20,
                            },
                            3: {
                                "user_id": 3,
                                "user_hash": 3,
                                "user_name": "Name 3",
                                "user_image": "",
                                "points": 0,
                                "position": 2,
                                "total_bets": 50,
                                "total_wins": 10,
                                "total_losses": 30,
                            },
                            4: {
                                "user_id": 4,
                                "user_hash": 4,
                                "user_name": "Name 4",
                                "user_image": "",
                                "points": 0,
                                "position": 2,
                                "total_bets": 50,
                                "total_wins": 10,
                                "total_losses": 30,
                            },
                            5: {
                                "user_id": 5,
                                "user_hash": 5,
                                "user_name": "Name 5",
                                "user_image": "",
                                "points": 0,
                                "position": 2,
                                "total_bets": 50,
                                "total_wins": 10,
                                "total_losses": 30,
                            },
                        }
                    }
                ]
            }
        }
	},
]
